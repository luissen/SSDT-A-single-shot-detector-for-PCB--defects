...
class MOD(nn.Module):
    ...
    def forward(self, x,test=False):
        ...
        for k in range(len(self.upper)-1):
            ...
            upper_source[lenscale-k-2] = bn(torch.cat([self.upper[lenscale-k-1](upper_source[lenscale-k-1]),scale_source[lenscale-k-2]],1))
        upper_source[0] = self.upper[0](upper_source[0])
...
def upper_deconv(vgg, extracts, size):    
    layers = []
    if size == 300:
        layers.append(BasicConv(512+512, 128*4, kernel_size=1, padding=0))
        layers+=[(BasicConv(vgg[-2].out_channels+128*2,512,kernel_size=1,padding=0,up_size = 38))]
        layers.append(BasicConv(extracts[1].out_channels+128,128*2,kernel_size=1,padding=0,up_size = 19))
        layers.append(BasicConv(extracts[3].out_channels+128,128,kernel_size=1,padding=0,up_size = 10))
        layers.append(BasicConv(extracts[5].out_channels+128,128,kernel_size=1,padding=0,up_size = 5))
        layers.append(BasicConv(extracts[7].out_channels,128,kernel_size=1,padding=0,up_size = 3))
    elif size ==512:
        layers.append(BasicConv(512+128*4, 128*4, kernel_size=1, padding=0))
        layers.append(BasicConv(vgg[-2].out_channels+128*2,512,kernel_size=1,padding=0,up_size = 64))
        layers.append(BasicConv(extracts[1].out_channels+128*2,128*2,kernel_size=1,padding=0,up_size = 32))
        layers.append(BasicConv(extracts[3].out_channels+128,128*2,kernel_size=1,padding=0,up_size = 16))
        layers.append(BasicConv(extracts[5].out_channels+128,128,kernel_size=1,padding=0,up_size = 8))
        layers.append(BasicConv(extracts[7].out_channels+128*1,128,kernel_size=1,padding=0,up_size = 4))
        layers.append(BasicConv(extracts[9].out_channels      ,128,kernel_size=1,padding=0,up_size = 2))
